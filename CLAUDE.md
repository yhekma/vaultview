...
- **Canvas rendering**: `.canvas` files are parsed as JSON server-side, emitted via `template.JS` to avoid HTML escaping, and rendered client-side with positioned divs + SVG edges. URL protocols are validated against an allowlist.
